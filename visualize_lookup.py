...
def visualize(L1, model_name):
    ...
    ax.plot(x, y, color=cols.to_rgb(colors[-i]))
    ax2.plot(x2, y2, color=cols.to_rgb(colors[-i]))
    ...
    ax.plot(x, y, color=cols.to_rgb(colors[-i-1]))
    ax2.plot(x2, y2, color=cols.to_rgb(colors[-i-1]))
    ...
    for i in range(0, airsps.size, 1):
        for n in [-1, 1]:
            ...
            ax.plot(x, y ,color=cols.to_rgb(colors[(2*i+n)]))
            ax2.plot(x2, y2, color=cols.to_rgb(colors[(2*i+n)]))
        ax.scatter(x[0], y[0], label=get_point(i), color=cols.to_rgb(colors[2*i]))
        ax2.scatter(x2[0], y2[0], label=get_point(i), color=cols.to_rgb(colors[2*i]))

    # min sink
    throt = L1[3, 0]
    pitch = L1[4, 0]*180/np.pi
    i = 5
    ax2.scatter(throt, pitch, label=get_point(i), color='green')
    # max climb
    throt = L1[3, 6]
    pitch = L1[4, 6]*180/np.pi
    i = 6
    ax2.scatter(throt, pitch, label=get_point(i), color='blue')



    ax.set_xlabel("Airspeed [m/s]")
    ax.set_ylabel("FPA [deg]")
    ax2.set_xlabel("Throttle [0,1]")
    ax2.set_ylabel("Pitch [deg]")



    ax2.legend(loc='best')
    plt.suptitle(f"{model_name} look-up table")
    plt.show()
